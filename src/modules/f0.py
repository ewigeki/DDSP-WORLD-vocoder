...
class F0Pipeline(L.LightningModule):
    def __init__(
            self,
            cent_predictor: nn.Module,
            sample_rate: int = 16000,
            target_downsample_factor: int | None = 4,
    ):
        super().__init__()
    ...
    def _align_target_f0(self, f0: torch.Tensor, target_length: int) -> torch.Tensor:
        if self.target_downsample_factor is not None:
            return f0[:, ::self.target_downsample_factor]
    ...
    def _shared_step(self, batch, batch_idx):
        ...
        f0_target = self._align_target_f0(f0, cent_pred_logits.size(1))
        loss = F.binary_cross_entropy_with_logits(cent_pred_logits, get_target(f0_target))
